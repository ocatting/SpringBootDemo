...
public class DateUtil {
	...
	/**
	 * 获取开始及结束日期间隔月份列表 start格式 "2015-01-01" end格式 "2016-01-01"
	 * 
	 * @param start
	 * @param end
	 * @return
	...
	public static List<String> getMonths(String start, String end) {
		...
		if (StringUtils.isNotBlank(start) && StringUtils.isNotBlank(end)) {
			Date dBegin = DateUtil.parse(start, DateUtil.PATTERN_YMD);
			Date dEnd = DateUtil.parse(end, DateUtil.PATTERN_YMD);
			...
			while (yearMonth2 >= yearMonth1) {
				dateList.add(DateUtil.format(calBegin.getTime(), DateUtil.PATTERN_YM));
	...
	public static Date parse(String date, String pattern) {
		...
		try {
			...
			logger.error("ToolDateTime.parse异常：date值" + date + "，pattern值" + pattern);
			return null;
		}
	}

	/**
	 * 获取开始及结束日期间隔年份列表 start格式 "2015-01-01" end格式 "2016-01-01"
	 * 
	 * @param start
	 * @param end
	 * @return
	...
	public static List<String> getYears(String start, String end) {
		...
		if (StringUtils.isNotBlank(start) && StringUtils.isNotBlank(end)) {
			...
			// 测试此日期是否在指定日期之后
			while (year2 >= year1) {
				dateList.add(DateUtil.format(calBegin.getTime(), "yyyy"));
	...
	public static List<String> getDay(String startDate, String endDate) {
		...
		try {
			...
			for (; dayc1.compareTo(dayc2) <= 0;) { // dayc1在dayc2之前就循环
				String month = null;
				String day = null;
				if (dayc1.get(Calendar.MONTH) + 1 < 10) {
					month = "0" + (dayc1.get(Calendar.MONTH) + 1);
				} else {
					month = dayc1.get(Calendar.MONTH) + 1 + "";
				}
				if (dayc1.get(Calendar.DATE) < 10) {
					day = "0" + dayc1.get(Calendar.DATE);
				} else {
					day = "" + dayc1.get(Calendar.DATE);
				}
				list.add(dayc1.get(Calendar.YEAR) + "-" + month + "-" + day);
	...
	private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ofPattern(FORMAT_ONE);

	public static LocalDate toLocalDate(String date){
		return LocalDate.parse(date,DateTimeFormatter.ofPattern(LONG_DATE_FORMAT));
	}
	...
	public static long toMilliSecond(String localDateTime){
		LocalDateTime time = toLocalDateTime(localDateTime);
		return toMilliSecond(time);
	}
